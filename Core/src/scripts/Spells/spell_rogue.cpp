        ...
        class spell_rog_preparation_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                for (SpellCooldowns::const_iterator itr = cm.begin(); itr != cm.end();)
                {
                    ...
                    {
                        if (classOptions->SpellFamilyFlags & SPELLFAMILYFLAG1_ROGUE_COLDB_SHADOWSTEP ||      // Cold Blood, Shadowstep
                            classOptions->SpellFamilyFlags & SPELLFAMILYFLAG_ROGUE_VAN_EVAS_SPRINT)           // Vanish, Evasion, Sprint
                            caster->ToPlayer()->RemoveSpellCooldown((itr++)->first, true);
                        else if (caster->HasAura(ROGUE_SPELL_GLYPH_OF_PREPARATION))
                        {
                            if (classOptions->SpellFamilyFlags & SPELLFAMILYFLAG1_ROGUE_DISMANTLE ||         // Dismantle
                                classOptions->SpellFamilyFlags & SPELLFAMILYFLAG_ROGUE_KICK ||               // Kick
                                (classOptions->SpellFamilyFlags & SPELLFAMILYFLAG_ROGUE_BLADE_FLURRY &&     // Blade Flurry
                                classOptions->SpellFamilyFlags & SPELLFAMILYFLAG1_ROGUE_BLADE_FLURRY))
                                caster->ToPlayer()->RemoveSpellCooldown((itr++)->first, true);
                            else
                                ++itr;
                        }
