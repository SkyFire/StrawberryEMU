...
bool EquippedOk(Player* pPlayer, uint32 spellId)
{
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        uint32 reqSpell = spellEffect->EffectTriggerSpell;
        if (!reqSpell)
            continue;
        ...
        for (uint8 j = EQUIPMENT_SLOT_START; j < EQUIPMENT_SLOT_END; ++j)
        {
                ...
                if (pItem->GetProto()->RequiredSpell == reqSpell)
            {
                ...
                sLog->outDebug(LOG_FILTER_SSCR, "SCR: player attempt to unlearn spell %u, but item %u is equipped.",reqSpell,pItem->GetProto()->ItemId);
                return false;
            }
        }
    }
