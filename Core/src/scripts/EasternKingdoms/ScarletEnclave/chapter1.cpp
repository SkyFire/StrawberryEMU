            ...
            case PHASE_TO_EQUIP:
                if (wait_timer)
                {
                    ...
                    else
                    {
                        ...
                        //sLog->OutDebug(LOG_FILTER_SSCR, "npc_unworthy_initiateAI: move to %f %f %f", anchorX, anchorY, me->GetPositionZ());
        ...
        if (pPlayer->GetQuestStatus(12701) == QUEST_STATUS_INCOMPLETE)
        {
            ...
            if (Creature *miner = pPlayer->SummonCreature(28841, 2383.869629f, -5900.312500f, 107.996086f, pPlayer->GetOrientation(),TEMPSUMMON_DEAD_DESPAWN, 1))
            {
                ...
                if (Creature *car = pPlayer->GetVehicleCreatureBase())
                {
                    ...
                    } else sLog->outError("SCR: OnGossipHello vehicle entry is not correct.");
                } else sLog->outError("SCR: OnGossipHello player is not on the vehicle.");
            } else sLog->outError("SCR: OnGossipHello Scarlet Miner cant be found by script.");
        }
        return true;
    }
