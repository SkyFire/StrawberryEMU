...
void Unit::SendMonsterMove(float NewPosX, float NewPosY, float NewPosZ, uint32 MoveFlags, uint32 time, float speedZ, Player *player)
{
    ...
    if (MoveFlags & SPLINEFLAG_UNKNOWN22)
    {
        data << uint8(0);
        data << uint32(0);
    }
        ...
        if (!bonus || coeff < 0)
        {
            ...
            if (damagetype == DOT)
            {
                ...
                if (DotDuration > 0)
                {
                    ...
                    for (uint8 j = 0; j < MAX_SPELL_EFFECTS; ++j)
                    {
                        SpellEffectEntry const* spellEffect = spellProto->GetSpellEffect(SpellEffIndex(j));
                        if (!spellEffect)
                            continue;
                    ...
                    if (spellEffect->EffectAmplitude != 0)
                        DotTicks = DotDuration / spellEffect->EffectAmplitude;
                    if (DotTicks)
                    {
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            ...
            if (pVictim)
            {
                if (!IsPositiveSpell(spellProto->Id))
                {
                    ...
                    //ApplyResilience(pVictim, &crit_chance);
                }
