...
struct SpellValue
{
    explicit SpellValue(SpellEntry const *proto)
    {
        ...
        {
            SpellEffectEntry const* spellEffect = proto->GetSpellEffect(SpellEffIndex(i));
            if (!spellEffect)
                continue;

            EffectBasePoints[i] = spellEffect->EffectBasePoints;
        }
...
namespace Strawberry
{
    struct SpellNotifierCreatureAndPlayer
    {
        ...
        {
            i_requireDeadTarget = i_spellProto ? bool(i_spellProto->AttributesEx3 & SPELL_ATTR_EX3_REQUIRE_DEAD_TARGET) : false;
