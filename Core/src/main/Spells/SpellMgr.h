...
inline bool IsSpellHaveEffect(SpellEntry const *spellInfo, SpellEffects effect)
{
    ...
    {
        if(SpellEffectEntry const* effectEntry = spellInfo->GetSpellEffect(SpellEffIndex(i)))
...
inline bool IsSealSpell(SpellEntry const *spellInfo)
{
    ...
    return classOptions && classOptions->SpellFamilyName == SPELLFAMILY_PALADIN &&
        (classOptions->SpellFamilyFlags & 0x26000C00
        || classOptions->SpellFamilyFlags & 0x0A000000);
}
...
inline bool IsElementalShield(SpellEntry const *spellInfo)
{
    ...
    return (classOptions && classOptions->SpellFamilyFlags & 0x420
        || classOptions->SpellFamilyFlags & 0x00000400)
        || spellInfo->Id == 23552;
}

inline bool IsExplicitDiscoverySpell(SpellEntry const *spellInfo)
{
    SpellEffectEntry const* spellEffect0 = spellInfo->GetSpellEffect(EFFECT_0);
    if (!spellEffect0)
        return false;

    SpellEffectEntry const* spellEffect1 = spellInfo->GetSpellEffect(EFFECT_1);
    if (!spellEffect1)
        return false;
...
inline bool IsSpellWithCasterSourceTargetsOnly(SpellEntry const* spellInfo)
{
    for (int i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        SpellEffectEntry const* effectEntry = spellInfo->GetSpellEffect(SpellEffIndex(i));
        if(!effectEntry)
            continue;
...
{
    SpellEffectEntry const* effectEntry = spellInfo->GetSpellEffect(SpellEffIndex(effIndex));
    if (!effectEntry)
        return false;

    if (IsAreaEffectTarget[effectEntry->EffectImplicitTargetA] || IsAreaEffectTarget[effectEntry->EffectImplicitTargetB])
        return true;
...
{
    SpellEffectEntry const* effectEntry = spellInfo->GetSpellEffect(EFFECT_0);
    if (!effectEntry)
        return false;

    //spellsteal is also dispel
    if (effectEntry->Effect == SPELL_EFFECT_DISPEL ||
        effectEntry->Effect == SPELL_EFFECT_DISPEL ||
        effectEntry->Effect == SPELL_EFFECT_DISPEL)
        return true;
    return false;
}
...
inline bool IsDispelSpell(SpellEntry const *spellInfo)
{
    ...
    //spellsteal is also dispel
    if (effectEntry->Effect == SPELL_EFFECT_STEAL_BENEFICIAL_BUFF ||
        effectEntry->Effect == SPELL_EFFECT_STEAL_BENEFICIAL_BUFF ||
        effectEntry->Effect == SPELL_EFFECT_STEAL_BENEFICIAL_BUFF
        ||IsDispel(spellInfo))
        return true;
    return false;
...
inline bool IsRangedWeaponSpell(SpellEntry const* spellInfo)
{
    ...
    return (classOptions->SpellFamilyName == SPELLFAMILY_HUNTER && !(classOptions->SpellFamilyFlags & 0x10000000)) // for 53352, cannot find better way
        || (equipItems && equipItems->EquippedItemSubClassMask & ITEM_SUBCLASS_MASK_WEAPON_RANGED);
}
...
{
    uint32 mask = 0;
    SpellEffectEntry const* spellEffect = spellInfo->GetSpellEffect(SpellEffIndex(effect));
    if (!spellEffect)
        return 0;
    ...
    if (spellEffect->EffectMechanic)
        mask |= 1 << spellEffect->EffectMechanic;
    return mask;
}
...
{
    SpellEffectEntry const* effectEntry = spellInfo->GetSpellEffect(SpellEffIndex(effect));
    SpellCategoriesEntry const* spellCategory = spellInfo->GetSpellCategories();
    if (effectEntry && effectEntry->EffectMechanic)
    ...
    public:
        ...
        inline bool IsSpellWithCasterSourceTargetsOnly(SpellEntry const* spellInfo)
        {
            for (int i = 0; i < MAX_SPELL_EFFECTS; ++i)
            {
                ...
                if (uint32 target = spellEffect->EffectImplicitTargetA)
