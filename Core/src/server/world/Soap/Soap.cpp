...
void SoapRunnable::run()
{
    ...
    while(!World::IsStopped())
    {
        ...
        sLog->outDebug(LOG_FILTER_NONE, "Soap: accepted connection from IP=%d.%d.%d.%d", (int)(soap.ip>>24)&0xFF, (int)(soap.ip>>16)&0xFF, (int)(soap.ip>>8)&0xFF, (int)soap.ip&0xFF);
...
int ns1__executeCommand(soap* soap, char* command, char** result)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_NONE, "Soap: Client didn't provide login information");
        return 401;
    }
    ...
    if(!accountId)
    {
        sLog->outDebug(LOG_FILTER_NONE, "Soap: Client used invalid username '%s'", soap->userid);
        return 401;
    }

    if(!sAccountMgr->CheckPassword(accountId, soap->passwd))
    {
        sLog->outDebug(LOG_FILTER_NONE, "Soap: invalid password for account '%s'", soap->userid);
        return 401;
    }

    if(sAccountMgr->GetSecurity(accountId) < SEC_ADMINISTRATOR)
    {
        sLog->outDebug(LOG_FILTER_NONE, "Soap: %s's gmlevel is too low", soap->userid);
        return 403;
    }
    ...
    sLog->outDebug(LOG_FILTER_NONE, "Soap: got command '%s'", command);
